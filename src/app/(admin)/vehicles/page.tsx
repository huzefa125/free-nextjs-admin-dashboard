...
export default function VehiclesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading("Saving vehicle info...");
    try {
      if (editingId) {
        ...
        toast.success("New vehicle added!", { id: loadingToast });
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        toast.success("Vehicle removed", { id: loadingToast });
        fetchData();
      } catch (err: any) {
    ...
    <div className="space-y-6">
              ...
              {loading ? <TableRow><TableCell colSpan={5} className="py-12 text-center text-gray-400">Loading vehicles...</TableCell></TableRow> : 
                vehicles.map(vh => (
                  <TableRow key={vh._id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                    <TableCell className="px-5 py-4 font-bold text-gray-900 dark:text-white/90 uppercase tracking-wider">{vh.registrationNumber}</TableCell>
                    ...
                    <TableCell className="px-5 py-4 text-end">
                      <div className="flex justify-end gap-1.5 h-full">
                        <button onClick={() => { setEditingId(vh._id); setFormData({ customerId: vh.customerId?._id || vh.customerId || "", model: vh.model, registrationNumber: vh.registrationNumber, status: vh.status, purchaseAmount: vh.purchaseAmount || 0, purchaseDate: vh.purchaseDate ? new Date(vh.purchaseDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0] }); setIsModalOpen(true); }} className="p-2 text-gray-400 hover:text-brand-500 rounded-lg transition-all" title="Edit Vehicle">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} className="max-w-[600px] p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <SearchableSelect label="Select Owner" options={customers.map(c => ({ label: c.name, value: c._id }))} 
